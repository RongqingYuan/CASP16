...
def bootstrap_sum(measures, model, mode,
    ...
    # heatmap_data = pd.DataFrame(np.random.rand(50, 10), columns=[f'R{i}' for i in range(1, 11)])
    ...
    # sorted_mask = mask[sorted_indices, :]  # 重新排序 mask
    ...
    breakpoint()
    ax0.set_yticklabels(
        [f'{i}' for i in sorted_indices], rotation=0)  # 使用排序后的标签
    ax0.set_xticklabels(sorted_heatmap_data.columns, rotation=90)
    ax0.set_title("Heatmap with Gray Masked Areas for Problematic Data")
