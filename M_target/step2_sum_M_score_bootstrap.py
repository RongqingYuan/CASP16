...
def bootstrap_sum(measures, model, mode,
    ...
    data = pd.DataFrame()
    measure_score_dict = {}
    EU_score_path = EU_score_path + f"impute={impute_value}/"
    ...
    convert = {
        ...
        "TMscore": "TMscore",
        "lDDT": "lDDT",
        "prot_nucl_per_interface_qs_global": "prot_nucl_interface_QSglob",
        "prot_nucl_per_interface_ics_trimmed": "prot_nucl_interface_ICS",
        "prot_nucl_per_interface_ips_trimmed": "prot_nucl_interface_IPS",
        "prot_per_interface_qs_global": "prot_prot_interface_QSglob",
        "prot_per_interface_ics_trimmed": "prot_prot_interface_ICS",
        "prot_per_interface_ips_trimmed": "prot_prot_interface_IPS",
    }  # mainly to look nice
    ...
    if equal_weight:
        plt.title(
            f"sum z-score for hybrid targets, interface score, equal weight", fontsize=20)
    ...
    # get the target list, it is the first element when split
    ...
    if equal_weight:
        plt.title("bootstrap result for hybrid target, interface score, equal weight".format(
...
parser.add_argument("--output_path", type=str,
                    # default="./bootstrap_prot_nucl_interface_score_v1/"
                    # default="./bootstrap_prot_prot_interface_score_v1/"
                    default="./bootstrap_interface_score_v1/"
                    )
