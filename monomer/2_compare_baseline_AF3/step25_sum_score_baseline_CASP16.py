...
print(data_raw.isna().sum(axis=0)/data_raw.shape[0])
print(data_raw.isna().sum(axis=1)/data_raw.shape[1])
# for each row, get its non-nan values in intersection with "TS304", and calculate the sum of the non-nan values
# then divide the sum of the non-nan values by the sum of "TS304" to get the normalized sum
...
# there is one group 304, we need to write something on top of its bar
for group, value in zip(groups, values):
    if group == highlight_group:
        plt.text(group, value + 0.03, str("AlphaFold3"),
...
plt.savefig(
    png_path + "sum_intersect_{}-{}-{}_with_af3_baseline.png".format(feature, model, mode), dpi=300)
###############
...
for index, value in zip(data_sum.index, data_sum.values):
    if index == highlight_index:
        plt.text(index, value - 5, str("AlphaFold3"),
...
data_raw_sum = data_raw_sum / data_raw_sum["304"]
plt.figure(figsize=(30, 15))
highlight_index = "304"
bar_colors = ['C0' if index !=
...
for index, value in zip(data_raw_sum.index, data_raw_sum.values):
    if index == highlight_index:
        plt.text(index, value + 2.0, str("AlphaFold3"),
