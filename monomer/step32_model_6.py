...
for feature in features:
    ...
    sns.heatmap(normalized_data, annot=True, fmt=".2f",
                cmap=cmap, norm=norm, square=True)
    plt.yticks(rotation=0, fontsize=12)
    plt.xticks(rotation=90, fontsize=12)
    plt.xlabel("Target", fontsize=12)
    plt.title(f"{feature} values with baseline_first", fontsize=20)
    plt.savefig(
    ...
    equal_to_1_count = (normalized_data == 1).sum(axis=1)
    equal_to_1_count = equal_to_1_count.sort_values(ascending=False)
    less_than_1_count = (normalized_data < 0.99).sum(axis=1)
    print("greater_than_1_count: ", greater_than_1_count)
    print(dict(greater_than_1_count))
    print("less_than_1_count: ", less_than_1_count)
